...
// };
​
// new solution: greedy check + binary search
class Solution {
public:
    int splitArray(const vector<int>& nums, int m) {
        int minMaxSubarraySum = 0;
        int maxMaxSubarraySum = 0;
        for (int i = 0; i < nums.size(); ++i) {
            minMaxSubarraySum = max({minMaxSubarraySum, nums[i]});
            maxMaxSubarraySum += nums[i];
        }
        
        int low = minMaxSubarraySum;
        int high = maxMaxSubarraySum;
        while (low < high) {
            int mid = low + (high - low) / 2;
            if (greedyCheck(nums, m, mid)) {
                high = mid;
            } else {
                low = mid + 1;
            }
 ...
    }
    
    bool greedyCheck(const vector<int>& nums, int m, const int maxSum) {
